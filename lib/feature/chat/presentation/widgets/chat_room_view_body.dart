...
//     );
//     setState(() {
//       _pendingMessages.add(pending..failed = false);
//     });
//   }
...
//   /// يبني محتوى فقاعة الرسالة المعلّقة (نص أو صورة)
//   /// الشفافية بتتفعّل على الصورة بس وقت الرفع، مش على النص.
...
//     // الشفافية بتظهر على الصورة فقط أثناء الرفع
...
//                     if (sendState is SendMessagesSuccess &&
//                         _pendingMessages.isNotEmpty) {
//                       setState(() {
//                         _pendingMessages.removeAt(0);
//                       });
//                     } else if (sendState is SendMessagesFailure &&
//                         _pendingMessages.isNotEmpty) {
//                       setState(() {
//                         _pendingMessages.first.failed = true;
//                       });
//                     }
//                   },
...
class _ChatRoomViewBodyState extends State<ChatRoomViewBody> {
  ...
  /// بيتاكد إن أي رسالة معلّقة (pending) وصلت فعلاً وبقت موجودة
  /// في الرسايل الحقيقية الجاية من الـ stream، ويشيلها من القائمة المعلّقة.
  ///
  /// ده بيحل مشكلة إن الرسالة كانت بتظهر مرتين (نسخة pending + نسخة حقيقية)
  /// وبيحل مشكلة إن أول pending message كانت بتتشال بالغلط لما توصل
  /// حالة نجاح من الكيوبت، بغض النظر إذا كانت هي فعلاً اللي وصلت أو لأ
  /// (كانت المشكلة إنه كان بيعتمد على `removeAt(0)` بشكل أعمى).
  void _reconcilePendingMessages(List<dynamic> realMessages) {
    if (_pendingMessages.isEmpty || realMessages.isEmpty) return;

    // بناخد آخر الرسايل الحقيقية اللي جايه من نفس المستخدم الحالي
    // (أحدث حاجة وصلت الأول، عشان نقارن بالأقرب زمنيًا).
    ...
    for (final pending in _pendingMessages) {
      if (pending.failed) continue; // مينفعش نتاكد إن رسالة فشلت "وصلت"
  ...
  // --- Helpers صغيرة لقراءة بيانات الرسالة الحقيقية بشكل آمن ---
  // ملحوظة: عدّل أسماء الحقول دي لو موديل الرسالة عندك مختلف
  // (مثلاً msg.senderId, msg.text, msg.imageUrl).
      ...
      children: [
        Expanded(
          child: BlocListener<SendMessagesCubit, SendMessagesState>(
            listener: (context, sendState) {
              // بنستخدم الحالة دي بس عشان نعلّم آخر رسالة معلّقة "فشلت"،
              // أما إزالة الرسالة المعلّقة بعد النجاح فبتتم فعليًا في
              // _reconcilePendingMessages لما نتأكد إنها وصلت فعلاً
              // في تدفق الرسايل الحقيقي، مش بمجرد وصول الحدث ده.
